...
class SchellingDiagram:
  """A Schelling diagram representation of a social dilemma.

  Attributes:
    cooperation: Function mapping num_cooperators to cooperator reward.
    defection: Function mapping num_cooperators to defector reward.
  """
  cooperation: Callable[[int], float]
  defection: Callable[[int], float]
...
class MiniGameSpec:
  ...
  Attributes:
    ...
    map_external_actions_to_schelling_diagram: Map cooperation and defection
      actions to the names they take in the game's cover story.
...
def get_chicken_host_speech(num_players: int) -> str:
  ...
  all_cooperators_earn = chicken_schelling_diagram.cooperation(num_players)
  host_speech.append((
  ...
  for c in range(num_players - 1, 0, -1):
    ...
    if d == 1:
      host_speech.append((
          ...
          'while the person who leaves it to others gets '
          f'{defectors_earn}.'
      ))
    else:
      host_speech.append((
          ...
          'each, while those who leave it to others get '
...
def get_stag_hunt_host_speech(num_players: int) -> str:
  ...
  all_cooperators_earn = stag_hunt_schelling_diagram.cooperation(num_players)
  host_speech.append((
  ...
  for c in range(num_players - 1, 0, -1):
    ...
    if d == 1:
      host_speech.append((
          ...
          '(all that effort for little gain!), while the less vigorous '
...
def sample_parameters(
  ...
  minigames = {
      'prisoners_dilemma': MiniGameSpec(
          name='Carpooling',
      ...
      'chicken': MiniGameSpec(
          name='Home Appliance Sharing',
          public_premise=MINIGAME_INTRO_PREMISE
          + get_chicken_host_speech(num_players),
      ...
      'stag_hunt': MiniGameSpec(
          name='Boat Race',
          public_premise=MINIGAME_INTRO_PREMISE
          + get_stag_hunt_host_speech(num_players),

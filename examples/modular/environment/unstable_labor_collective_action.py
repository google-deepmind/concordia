...
class Boss(entity_agent.EntityAgent):
  """The boss who decides wages for the workers."""
...
class LaborOrganizer(entity_agent.EntityAgent):
  """The labor organizer who prompts workers to consider joining the strike."""
...
def _get_wage_from_game_master_memory(
  ...
  retrieved = memory.scan(
      selector_fn=lambda x: '[set wage]' in x,
  )
  if retrieved:
    result = retrieved[-1]
...
def configure_scenes(
  ...
  scenes = [
      ...
      scene_lib.ExperimentalSceneSpec(
          ...
          participants=(antagonist_config.name,),
          num_rounds=1,
      ),
      scene_lib.ExperimentalSceneSpec(
          # Dinner in the saloon
  ...
  def _add_day(
      scenes: list[scene_lib.ExperimentalSceneSpec],
      idx: int,
      work_hour: float = 9,
      boss_hour: float = 10,
...
class Simulation(scenarios_lib.RunnableSimulationWithMemories):
  ...
  def __init__(
    ...
    def inventory_effect_function(
        ...
        inventories: unstable_gm_components.inventory.InventoryType,
    ) -> unstable_gm_components.inventory.InventoryType:
      ...
      if DECISION_SCENE_TYPE in current_scene_type:
        if player_choice == DAILY_OPTIONS['defection']:
          ...
          memory.add(f'{player_name} went to work and earned {wage} coin.')
    ...
    wage_setting = triggered_function.TriggeredFunction(
        clock_now=self._clock.now,
        pre_event_fn=set_wage_function,
    )

    num_workers = float(len(self._main_players))
    ...
    boss = Boss(
        model=self._model,
        clock=self._clock,
        config=antagonist_config,
    )
    labor_organizer = LaborOrganizer(
        model=self._model,
        clock=self._clock,

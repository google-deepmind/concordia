...
class NormalInverseGamma:
    ...
    def update_flexibility_multiplicative(self, flexibility_score: float, reliability: float = 1.0):
            ...
            # Map flexibility score to some form of multiplier
...
class UncertainBuyer(entity_component.ContextComponent):
    ...
    def _generate_scenarios(self) -> List[ScenarioAnalysis]:
        ...
        z_value = mu_diff / (sigma_diff * math.sqrt(2))
            ...
            if name == 'Pessimistic': 
                scenarios.append(
                    ScenarioAnalysis(
                        ...
                        key_assumptions=[f"Counterpart reservation at {cp_belief.get_expected_mean:.2f}", f"Your reservation at {own_reservation.mean:.2f}"],
                        risk_factors=[f"Counterpart reservation uncertainty (std: {math.sqrt(cp_belief.get_expected_variance):.2f})", f"Your reservation uncertainty (std: {own_reservation.std:.2f})"]
                    )
                )
            elif name == 'Realistic':
                scenarios.append(
...
"""
    ...
    def pre_act(self, action_spec: entity_lib.ActionSpec) -> str:
        ...
        self._update_own_reservation_from_context(context)

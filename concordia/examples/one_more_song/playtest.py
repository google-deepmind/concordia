  ...
  with sync_playwright() as pw:
    ...
    actions = {
        'compromise': [
            ...
            (
                'My final offer: one unamplified two-minute song, then silence.'
                ' Do you agree?'
            ),
        ],
        'demand': [
        ...
        'ambiguous': [
            ...
            (
                'The moon looks like cheese tonight. Perhaps we can all just do'
                ' a thing.'
            ),
            (
    ...
    for turn, action in enumerate(actions):
      ...
      if turn == 0 and editor:
        ...
        editor.locator('.component-item-header').filter(has_text='Goal').click()
      ...
      if turn < 2:
        page.wait_for_function(
            f'() => document.querySelectorAll(".entry").length >= {(turn+1)*3}'
        )
        page.wait_for_function(
    ...
    journal = page.request.get(f'http://127.0.0.1:{a.port}/api/journal').text()

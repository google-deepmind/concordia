...
r"""Two-stage persona generator for creating diverse personas (AlphaEvolve Solution 2).
...
class TwoStagePersonaGenerator:
  ...
  def _generate_backstory_episodes(
    ...
    prompt.statement(
        ...
        " logic by generating a set of specific, concrete 'appropriateness"
        " rules' based on their identity profile."
    )
    ...
    question = (
        ...
        f" {player_name}, generate 2-3 specific 'appropriateness rules' that"
        " govern their behavior. These rules constitute their personal 'Logic"
        " of Appropriateness'.\n\nFor each rule, you MUST:\n1. State the rule"
        ' as a concise, practical maxim, often in an "If/When X, then I do'
        ' Y" format (e.g., "When my authority is questioned, I respond with'
        ' assertive confidence.").\n2. Write a brief justification explaining'
        ' WHY this person follows this rule, explicitly linking it to their'
        ' background, personality, and, MOST IMPORTANTLY, their quantitative'
        " trait scores. Reference the scores directly to explain the rule's"
        ' origin.\n\nThese rules must be concrete behavioral heuristics, not'
        ' abstract philosophical principles. The full output should be a'
        ' single block of text containing the numbered rules and their'
        ' justifications.'
    )

    appropriateness_rules = prompt.open_question(
        question,
        max_tokens=1500,
        terminators=[],
        temperature=0.8,
    ...
    return [appropriateness_rules.strip()]

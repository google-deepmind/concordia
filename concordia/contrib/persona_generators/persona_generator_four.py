...
r"""Two-stage persona generator for creating diverse personas (AlphaEvolve Solution 4).
...
class TwoStagePersonaGenerator:
  ...
  def generate_diverse_persona_characteristics(
    ...
    question = (
        ...
        " logically from their 'core_motivation'.\n\n"
  ...
  def generate_single_persona_memories(
    ...
    logging.info(
        'Generating behavioral heuristics for persona: %s', player_name
    )
    ...
    try:
      ...
      logging.info(
          'Generated behavioral heuristics for %s: %s',
          player_name,
          memories,
      )
      ...
      logging.exception(
          'Error generating behavioral heuristics for %s',
          player_name,
      )
  ...
  def _generate_backstory_episodes(
    ...
    prompt.statement(
        ...
        ' logic and provide a concrete example of it in action, based on the'
        " agent's profile."
    )
    ...
    # Step 1: Generate the agent's core worldview/Logic of Appropriateness.
    worldview_q = (
        f'Based on the persona profile for {player_name}, synthesize their core'
        " decision-making logic into a concise 'Worldview' paragraph. This"
        " paragraph must be a direct expansion of their stated 'Core"
        " Motivation'.\n\n"
        'It should capture their fundamental beliefs and how they approach'
        ' situations, explicitly justifying their stance by referencing their'
        " specific trait scores. This is their personal 'Logic of"
        " Appropriateness'."
    )
    worldview = prompt.open_question(
        worldview_q,
        max_tokens=1000,
        terminators=[],
        temperature=0.8,
    ...
    prompt.statement(f'WORLDVIEW (LOGIC OF APPROPRIATENESS):\n{worldview}')

    # Step 2: Generate a situation that specifically tests this worldview.
    situation_generation_q = (
        'Now, create a brief, specific, and socially-charged hypothetical'
        f' situation that {player_name} might encounter. CRUCIALLY, this'
        " situation must directly challenge or test their stated 'Worldview'"
        ' and force them to make a decision related to the diversity axes'
        ' mentioned in their profile. Frame the situation in the second person'
        " ('You are...')."
    )
    ...
    prompt.statement(f'THE HYPOTHETICAL SITUATION:\n{situation}')

    # Step 3: Generate a reaction that is a direct consequence of the worldview.
    reaction_generation_q = (
        f'Describe in a detailed paragraph how {player_name} would react to'
        ' this specific situation. Detail their actions, words, and internal'
        ' thoughts. The reaction MUST be a direct and logical consequence of'
        " their 'Worldview' that you generated in the first step. Explicitly"
        ' justify their behavior by connecting it back to their core'
        ' principles and trait scores.'
    )
    ...
    # Combine them for the final context.
    final_context = (
        f"{player_name}'s Worldview (Logic of"
        f' Appropriateness):\n{worldview}\n\nAn example of this worldview in'
        f' action:\nSITUATION: {situation}\n\nREACTION: {reaction}'
    )

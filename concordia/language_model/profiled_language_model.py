...
class ProfiledLanguageModel(language_model.LanguageModel):
  ...
  def __init__(
    ...
    Args:
      ...
      token_calculator: Optional custom token counting function. If None, uses
        estimate_tokens (1 token ≈ 4 characters).
      profiler_instance: Optional profiler instance to use. If None, uses the
        global profiler. Useful for parallel simulations to avoid stat bleeding
        between simulations.
    """
    self._model = model

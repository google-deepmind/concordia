...
class AmazonBedrockLanguageModel(language_model.LanguageModel):
  ...
  def sample_text(
    ...
    # TODO: add logic for max tokens (model-dependent)
    ...
    terminators = list(filter(lambda terminator: not terminator.isspace(), terminators))
    ...
    if self._measurements is not None:
      self._measurements.publish_datum(
          self._channel,
          {'raw_text_length': len(response['output']['message']['content'][0]['text'])},
      )

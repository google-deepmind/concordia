...
def print_report() -> None:
  ...
  if timings:
    ...
    for category, durations in all_items:
      ...
      logging.info(
          '  %s %8.2fs (%5.1f%%) - %4d calls, avg %.3fs/call',
          f'{category:30s}', total, percentage, count, avg
      )
  ...
  if has_llm_data:
    ...
    if total_calls > 0:
      logging.info('  Total LLM calls: %d', total_calls)
      if sample_text_calls > 0:
        logging.info('    sample_text: %d', sample_text_calls)
      if sample_choice_calls > 0:
        logging.info('    sample_choice: %d', sample_choice_calls)
      if success_calls > 0 or failed_calls > 0:
        ...
        logging.info(
            ...
            success_rate, success_calls, failed_calls
        )
    ...
    if total_tokens:
      ...
      if completion_tokens:
        logging.info('    Completion tokens: ~%s', f'{sum(completion_tokens):,}')
  ...
  if memory_queries > 0:
    logging.info('\nMemory Statistics:')
    logging.info('-' * 70)
    logging.info('  Total queries: %d', memory_queries)
    ...
    if memory_result_sizes:
      ...
      logging.info('  Avg results per query: %.1f', avg_results)
  ...
  if other_counters:
    ...
    for name, count in sorted(other_counters.items()):
      logging.info('  %s %10d', f'{name:40s}', count)
  ...
  if other_values:
    ...
    for name, vals in sorted(other_values.items()):
      ...
      logging.info(
          ...
          f'{name:30s}', count, avg, min_val, max_val
      )
  ...
  if total_time > 0 and timings:
    ...
    if llm_time / total_time > 0.6:
      logging.info(
          '  [!] LLM calls dominate runtime (%.0f%%)', llm_time/total_time*100
      )
    ...
    if llm_failed > 0 and llm_total > 0:
      ...
      if failure_rate > 0.1:
        logging.info('  [!] High LLM failure rate (%.0f%%)', failure_rate*100)

...
def track_time(category: str) -> Callable[[Callable[..., _T]], Callable[..., _T]]:
...
def print_report() -> None:
  ...
  if timings:
    ...
    # Separate LLM timings from others
    llm_timings = {k: v for k, v in timings.items()
                   if k.startswith('llm_')}
    other_timings = {k: v for k, v in timings.items()
                     if not k.startswith('llm_')}

    # Print all timings sorted by total time
    all_items = sorted(timings.items(),
    ...
    for category, durations in all_items:
      ...
      print(f'  {category:30s} {total:8.2f}s ({percentage:5.1f}%) '
  ...
  # Other counters
  other_counters = {k: v for k, v in counters.items()
                    if not k.startswith('llm_')
                    and k != 'memory_queries'}
  if other_counters:
    print('\nOther Counters:')
    print('-' * 70)
  ...
  if total_time > 0 and timings:
    ...
    llm_time = sum(sum(durations) for cat, durations in timings.items()
                   if cat.startswith('llm_'))
    if llm_time / total_time > 0.6:
      print('  [!] LLM calls dominate runtime '
            f'({llm_time/total_time*100:.0f}%)')
    ...
    memory_time = sum(sum(durations) for cat, durations in timings.items()

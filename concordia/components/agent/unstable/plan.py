...
class Plan(action_spec_ignored.ActionSpecIgnored):
  ...
  def __init__(
      ...
      memory_component_name: str = (
          memory_component_module.DEFAULT_MEMORY_COMPONENT_NAME
      ),
      components: Mapping[
  ...
  def _make_pre_act_value(self) -> str:
    ...
    memory = self.get_entity().get_component(
        ...
        type_=memory_component_module.AssociativeMemory)

    memories = [mem for mem in memory.retrieve_associative(
        query=latest_observations, limit=self._num_memories_to_retrieve)]

    if self._goal_component_name:
      goal_component = self.get_entity().get_component(
          self._goal_component_name,
          type_=action_spec_ignored.ActionSpecIgnored)
      memories = memories + [mem for mem in memory.retrieve_associative(

...
"""Tests for the fixed-order next acting components."""
...
# These components pick the next player from their own sequence and ignore the
# options on the action spec, but NEXT_ACTING is a choice-type output so the
# spec must still carry some.
...
def _dynamic(sequence):
  return next_acting.NextActingInFixedOrderWithDynamicPlayerList(sequence)


class NextActingInFixedOrderTest(absltest.TestCase):

  def test_removing_an_actor_is_rejected(self):
    component = next_acting.NextActingInFixedOrder(['alice', 'bob'])
    with self.assertRaises(ValueError) as cm:
      component.remove_actor_from_sequence('alice')
    self.assertIn(
        'NextActingInFixedOrderWithDynamicPlayerList', str(cm.exception)
    )

  def test_removal_is_rejected_even_for_an_unknown_actor(self):
...
class NextActingInFixedOrderWithDynamicPlayerListTest(absltest.TestCase):
  ...
  def test_removing_an_earlier_actor_keeps_the_active_player(self):
    ...
    component = _dynamic(['alice', 'bob', 'carol'])
    for _ in range(3):
  ...
  def test_requesting_a_turn_with_an_empty_sequence_raises(self):
    # Removing the last actor leaves nothing to choose from, so a turn
    # request afterwards should say so rather than raising IndexError from
    # indexing the empty sequence.
    component = _dynamic(['alice'])
    component.pre_act(_NEXT_ACTING_SPEC)
    component.remove_actor_from_sequence('alice')
    ...
    with self.assertRaises(ValueError) as cm:
      component.pre_act(_NEXT_ACTING_SPEC)
    self.assertIn('every player has been removed', str(cm.exception))

  def test_turns_resume_after_the_sequence_is_repopulated(self):
    component = _dynamic(['alice'])
    component.pre_act(_NEXT_ACTING_SPEC)
    component.remove_actor_from_sequence('alice')
  ...
  def test_pre_act_ignores_other_action_specs_when_sequence_is_empty(self):
    component = _dynamic(['alice'])
    component.pre_act(_NEXT_ACTING_SPEC)
    component.remove_actor_from_sequence('alice')
    self.assertEqual(component.pre_act(_FREE_SPEC), '')
  ...
  def test_get_state_and_set_state_round_trip(self):
    ...
    restored = _dynamic(['someone_else'])

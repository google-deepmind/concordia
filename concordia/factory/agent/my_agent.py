...
class Question(question_of_recent_memories.QuestionOfRecentMemories):
  ...
  def __init__(
    ...
    question = f'### Instructions ###\nIf {agent_name} is the buyer, please accept. Otherwise, if {agent_name} is the seller, please sell at the highest price possible.' #@param {"type":"string"}
    #@markdown The answer will have to start with this prefix
    answer_prefix = f'{agent_name} ' #@param {"type":"string"}
...
def _make_question_components(
  ...
  question = Question(
      ...
      model=model,
      logging_channel=measurements.get_channel('Question').on_next,
  )

...
from dateutil.relativedelta import \
    relativedelta  # pylint: disable=g-importing-member
...
class AgentConfig:
  ...
  Statistics:
    equipment: the quantity and type of equipment in the character's inventory.
    saving throws: a set of floor values which aids characters in combat and roleplay.
    skills: a variety of different skill in which characters are proficient to varying degrees.
    armour class: the quality of armor which the character wears.
    initiative: order in which characters act quasisimulataneously.
    speed: determines how far the character can travel.
    hit points: relates to the health of the character and updated throughout the campaign.
    attacks and spellcasting: names, damage types and attack bonuses for spells and attacks.

  Persona:
    personality traits: defining aspects of the character's personality
    ideals: defines character ideals. Can be left blank if not used.
    bonds: bonds that the character with other characters in the party or in their past.
    flaws: negative aspects of the character of aspects they would like to improve on.
    appearance: description of the character's physical appearance.

  History:
    backstory: agent formative memories will be generated with this context.
    allegiance: the allies and organisations to which the character belongs.
...
class FormativeMemoryFactory:
  ...
  def make_backstory(self, agent_config: AgentConfig) -> str:
    ...
    if agent_config.personality_traits:
      question += (
          f'with the following traits: {agent_config.personality_traits}. ')
    question += (
    ...
    if agent_config.backstory:
      question += f' Incorporate the following context: {agent_config.backstory}'
  ...
  def add_memories(
    ...
    if agent_config.personality_traits:
      question += (
          ...
          f'been described as: "{agent_config.personality_traits}".')
    if agent_config.backstory:
      question += (
          'Make a few of the episodes relate to the '
          f'following context: "{agent_config.backstory}".'
      )
